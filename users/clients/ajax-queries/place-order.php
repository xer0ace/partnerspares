    ...
    else
    {
        $checkID = "SELECT * FROM `tbl_to_pay` WHERE Date_Added = '$dateQuery' ORDER BY To_Pay_ID  DESC LIMIT 1 ";
            ...
            if (mysqli_num_rows($checkResult) == 0) {
               ...
               $link->query("INSERT INTO `tbl_to_pay` VALUES ('$To_Pay_ID', '$Client_ID', '$To_Pay_Items', '$item_quantity' , '$total_price', '$dateQuery', 'Dine In', '$table')");

                $link->query("UPDATE `tbl_dine_table` SET `Status` = 'Occupied' WHERE `tbl_dine_table`.`Table_Number` = '$table'");
